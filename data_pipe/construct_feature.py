...
def extract_feature(stock_name, market_name, date, previous_date, data_path):
    ...
    if stock_name not in ['Mkt-RF', 'SMB', 'HML', 'RMW', 'CMA']:
        kline_data = pd.read_csv(
            f'{data_path}/kline_day/{market_name}/{stock_name}.csv', index_col='date')
...
def one_day(date, market_name, data_path, save_path, stock_list, date_array, horizon=1, num_worker=20):
    previous_date = date_array[np.where(date_array == date)[
        0].item() - lag_order + 1]
...
for date in tqdm(selected_date_array, desc='construct feature'):
