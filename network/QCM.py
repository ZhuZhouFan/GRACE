...
def compute_QCM_table(stock_name, inference_dict, 
    ...
    selected_tau_list = np.array([x for x in quantile_df.columns.values if (x != 'ground_truth')])
    
    # log_ret to ret
    quantile_df = np.exp(quantile_df) - 1
    
    result_table = QCM_regression(quantile_df)
    return result_table, selected_tau_list


def compute_CoMoments_table(stock_name,
                            inference_dict,
                            selected_tau_list,
                            tolerance=20,
                            ):
    
    if len(selected_tau_list) <= tolerance:
        print(f'There are too little taus in {stock_name}')
        return None
    stock_df = re_arrange(stock_name, selected_tau_list, inference_dict)
    stock_df.set_index('date', inplace=True)
    quantile_df = stock_df[selected_tau_list + ['ground_truth']]
